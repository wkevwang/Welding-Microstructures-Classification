...
class FCN8VGG:
    ...
    def __init__(self, vgg16_npy_path=None):
        ...
        if not os.path.isfile(vgg16_npy_path):
            print(("File '%s' not found. Download it from "
    ...
    def build(self, rgb, train=False, num_classes=20, random_init_fc8=False,
              debug=False, use_dilated=False, dropout_keep_prob=1.0):
        """
        ...
        if train:
            self.fc6 = tf.nn.dropout(self.fc6, dropout_keep_prob)

        self.fc7 = self._fc_layer(self.fc6, "fc7")
        if train:
            self.fc7 = tf.nn.dropout(self.fc7, dropout_keep_prob)
